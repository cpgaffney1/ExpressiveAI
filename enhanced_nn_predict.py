...
def computeActualFromOffset(originalMusStarts, predictions):
    computed = []
    for i in range(len(predictions)):
        tup = predictions[i]
        origStart = originalMusStarts[i]
        computed.append([tup[0] + origStart, tup[1]])
    return computed

def predict(seq_model, chord_model=None, core_input_shape=5, withOffset=False):
    ...
    for i in range(len(rec_x_test)):
        ...
        if core_test_features[i][0] == core_test_features[i][1] and chord_model != None or chord_model == None:
    ...
    predictions = computeActualFromOffset(originalMusStarts, predictions)
...
def main():
    ...
    ################
    if oneModel:
        model = load_model('enhanced_nn_model.h5')
        ###
        '''
